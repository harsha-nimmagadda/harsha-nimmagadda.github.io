...
// ============================================================
// Pick a student from the batch → render their percentile, deltas
// vs batch median, gap to top scorer, and per-subject deltas.
// Reuses the existing /api/v1/users/students/search endpoint
// (scoped by batchId so the picker only suggests batch members).
...
export function StudentBatchRank({ batchId }: { batchId: string }) {
  ...
  const searchQuery = useQuery({
    queryKey: ["batch-student-search", batchId, debouncedQuery],
    enabled: debouncedQuery.trim().length >= 2 && !picked,
    queryFn: async () => {
      const res = await apiClient.get<StudentSearchResult[]>(
        `/api/v1/users/students/search?q=${encodeURIComponent(debouncedQuery)}&batchId=${encodeURIComponent(batchId)}&limit=10`,
      );
  ...
  const rankQuery = useQuery({
    queryKey: ["batch-student-rank", batchId, picked?.id],
    enabled: !!picked?.id,
    queryFn: async () => {
      const res = await apiClient.get<StudentRankResponse>(
        `/api/v1/analytics/v3/batch/${batchId}/student-rank?studentId=${picked!.id}`,
      );
      ...
      <Card hoverable={false} padding="md">
        <CardHeader className="pb-2">
          ...
          <p className="text-xs text-muted">
            Search a student in this batch — see their percentile, gap to top,
            and which subjects they're behind/ahead of batch peers.
          </p>
        </CardHeader>
        <CardContent className="space-y-4 pt-2">
            ...
            {!picked &&
              ...
              !searchQuery.isLoading && (
                <p className="mt-1 px-2 text-xs text-muted">
                  No students in this batch match "{debouncedQuery}".
                </p>
              )}
          </div>
          ...
          {picked && rankQuery.data && <RankResult data={rankQuery.data} />}
        </CardContent>
      </Card>
    </motion.div>
  );
...
function RankResult({ data }: { data: StudentRankResponse }) {
  if (data.batchSize === 0 || data.studentAvgPct === null) {
    return (
      <div className="rounded-2xl border border-dashed border-border bg-white p-6 text-center dark:border-border-dark dark:bg-surface-dark">
        <p className="text-sm font-medium text-foreground">
          No exam data for {data.studentName} yet
        </p>
        <p className="mt-1 text-xs text-muted">
          The rank populates after this student has at least one submitted exam.
        </p>
      </div>
    );
  }
    ...
    <div className="space-y-4 rounded-2xl border border-border bg-white p-5 dark:border-border-dark dark:bg-surface-dark">
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <p className="text-xs text-muted">{data.batchName}</p>
        </div>
        <Badge variant="info" size="sm">
          Rank #{data.rankInBatch} / {data.batchSize}
        </Badge>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
        <Kpi
          ...
          sub={`batch median ${data.batchMedianPct ?? "—"}%`}
        />
        <Kpi
          label="Percentile"
        ...
        <Kpi
          label="Gap to top"
          value={data.gapToTop !== null ? `${data.gapToTop} pts` : "—"}
          sub={
            data.topScorer ? (
              <Link
      ...
      {(ahead.length > 0 || behind.length > 0) && (
        <div className="grid gap-3 border-t border-border pt-3 sm:grid-cols-2 dark:border-border-dark">
          <div>
            ...
            {ahead.length === 0 ? (
              <p className="text-xs text-muted">No subject above batch avg.</p>
            ) : (
          ...
          <div>
            ...
            {behind.length === 0 ? (
              <p className="text-xs text-muted">No subject below batch avg.</p>
            ) : (
              <ul className="space-y-1 text-sm">
                {behind.map((s) => (
                  <li key={s.subjectName} className="flex items-center justify-between gap-2">
